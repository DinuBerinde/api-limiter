...
public class ApiLimiterSpecificClientTest {
    ...
    @Test
    @DisplayName("MaxCalls = 5, Interval = 5sec -> Should fail consuming 2 calls out of 12 calls: 5 calls + interval reset + 5 calls + 2 calls")
    public void shouldFailOn12CallsWithIntervalReset() throws InterruptedException {
        ...
        for (int i = 1; i <= 10; i++) {
            ...
            if (i == 6) {
                Thread.sleep(5 * 1000); // let the interval reset
            }
            consumers.add(ApiLimiter.consume(API_NAME, CLIENT));
        }
